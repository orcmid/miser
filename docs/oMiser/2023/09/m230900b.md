<!-- m230900b.md 0.1.4              UTF-8                         2026-01-25
     ----1----|----2----|----3----|----4----|----5----|----6----|----7----|--*
...
<table border="0" width="100%">
  <tr>
    ...
    <td width="27%" height="6" valign="middle" align="right">
      ...
      <br />
      <small><small>
        0.1.4 2026-01-25T23:11Z<!-- MAINTAIN THIS MANUALLY -->
      </small></small>
      </td>
  </tr>
</table>
...
released as long as the oMiser instance persists.  Because there is such
identifiability, including re-creation of a Synthetic, the condition
essentially holds for all individuals, no matter their origin or current
accessibility.

It appears that it must be possible to find individuals by their HashFlags,
because the resolution of their definition requires it.  In effect, it is
for access to the definition of the individual, whether by its synthesis
or its literal identifier.  In both cases, the hash of a prospective
first-occurrence is used to access existing definitions and determine if there
already is one.

In all of these case, and only these, is the MOb required. The one difference
is that Synthetics have no individual name.

There is a lingering case with respect to bindings.  Bindings are naturally
externally-identifiable (and generally only so associated) and they do not
resolve to individuals, *as such*.  It is undesirable to determine that an
ob might be the the object of a binding by searching for it in the MOb. On
the other hand, it would be handy to know that about an ob-cell. To address
that temptation, one must reconcile the fact that an ob-cell can be the
object of more than one binding, and that a binding can be removed or
re-assigned in the course of operation.  The pay-off would be the ability
to abbreviate displayed obs by identifying (current) bindings.  This may
simply be far-fetched.
