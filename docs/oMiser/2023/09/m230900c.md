<!-- m230900c.md 0.0.6              UTF-8                         2026-02-16
     ----1----|----2----|----3----|----4----|----5----|----6----|----7----|--*
...
<table border="0" width="100%">
  <tr>
    ...
    <td width="27%" height="6" valign="middle" align="right">
      ...
      <br />
      <small><small>
        0.0.6 2026-02-16T18:05Z<!-- MAINTAIN THIS MANUALLY -->
      </small></small>
      </td>
  </tr>
</table>
...
Note: The term "distinction" is associated with the primitive individual,
`obap.d` used in ob scripts to determine if two obs are the same or different.
Individual `obap.a` is returned if the obs are identical and `obap.b` is
returned if they are different. The convention is to associate `obap.a` with
"true" and "distinction" is not meant in the sense of "different."  (This
would have been clearer if `obap.e` were not already taken.  Replacement of
`obap.d` with `obap.eq` is probably worthwhile, since the idea of single-
letter primitive names has already been abandoned.)
...
## 5. The Quest for Duplicate Elimination
...
Reference count adjustments are not reflected. On exit true, one
goes up, one goes down.  That can be handled in oldest(*x*, *y*).
