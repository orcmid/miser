<!-- m230900c.md 0.1.0              UTF-8                         2026-02-19
     ----1----|----2----|----3----|----4----|----5----|----6----|----7----|--*
...
<table border="0" width="100%">
  <tr>
    ...
    <td width="27%" height="6" valign="middle" align="right">
      ...
      <br />
      <small><small>
        0.1.0 2026-02-19T00:33Z<!-- MAINTAIN THIS MANUALLY -->
      </small></small>
      </td>
  </tr>
</table>
...
represented obs are of the same structure, whether or not the Hash part is
a collision.
...
When we look into the memory structures and connections among ob-cells, the
considerations are at the computer-implementation level and not at the level
of abstraction that Frugalese and the mathematical.  Simplifying ob-cell
structure to just enough for treating comparison, here is a C/C++ counterpart
of (2).
...
ob-cells whose obs are to be compared?  In (3) we have pointers to the
ob-cells as parameter values, but there is no knowledge of where those
pointers were obtained.

There are ones where we do know where we're coming from: going into
recursive steps `q(a(x), a(y))` and `q(b(x), b(y))` in (2) and its
counterpart (3) above.
...
adjusted when the corresponding ob-cell parts are found to be for the same ob.
...
ob* preferrable(ob *x, ob *y);  // some means of choosing between equals

bool qa(ob **px, ob **py)
  { if ( !q(a(*px), a(*py)) ) return false;
    px->a = py->a = preferrable(*px, *py);
    return true;
    }

bool qb(ob **px, ob **py)
  { if ( !q(b(*px), b(*py)) ) return false;
    px->b = py->b = preferrable(*px, *py);
    return true;
    }
...
The function ob *preferrable(ob *x, ob *y) returns a pointer to the ob that
...
It is unclear that this is worthwhile making aditional provisions for, until
case that arise with oFrugal might provide more perspective.
