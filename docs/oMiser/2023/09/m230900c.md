<!-- m230900c.md 0.0.10             UTF-8                         2026-02-17
     ----1----|----2----|----3----|----4----|----5----|----6----|----7----|--*
...
<table border="0" width="100%">
  <tr>
    ...
    <td width="27%" height="6" valign="middle" align="right">
      ...
      <br />
      <small><small>
        0.0.10 2026-02-18T16:47Z<!-- MAINTAIN THIS MANUALLY -->
      </small></small>
      </td>
  </tr>
</table>
...
* If HashFlags are the same, then different ob-cells *might* be for identical
  obs or else just be a HashFlag collision on different ob-cells.  If that
  identity is confirmed, however, there is potential for removing
...
Usage of hash codes to quickly distinguish list structures, as in LISP,
...
### 5.1 Knowing Where From

To eliminate a duplicate, we need to know where the references for *x* and *y*
were obtained so those references can be made to refer to the same ob-cell.
That is, where where are the separate ob-cells that are for the same ob
referenced such that both references can be made to just one of them, reducing
duplication.
...
Replace those with `qa(px, py)` and `qb(px, py)`, passing in references to
references to the ob-cells for the obs to be compared.  That is enoogh
to determine how to coalesce matches of obs in different locations.

In very-pseudo-Frugalese,

  \[AUTHOR NOTE: This attempt at introduction of storage assignment is
                 simply not something Frugalese is any good for, and
                 everything is too inscrutible.  I must replace this with
                 a C/C++ version that is not the actual oMiser implementation
                 but that conveys all of the key ideas.  Working on it.
                 The following material will be replaced.\]
...
Assignment (:=) is not good Frugalese but it is needed in describing such
low-level stateful operations.
...
* Note that q(*x*, *y*) is used in these procedures so there is no benefit
  at deeper levels. That can be remedied.
...
### 5.3 Can Greed Be Good?

At an initial entry to q(*x*, *y*), there is nothing helpful at that
presumably top-level case.  That can be handled similarly to the introduction
of qa and qb.
...
It is the idea that matters, not the purity of this pseudo-code.


## 6. Experimentation and Instrumentation

It takes much instrumentation and testing to determine if reducing different
ob-cells for identical obs is worthwhile.

Better ways of describing the comparisons are desirable.  Frugalese is too
functional to express state-changing activity at a level where it is all meant
to be invisible in oMiser manifestabion of the Ob universe.  The C/C++
implementation will be clear.
...
<!--
  ...
  0.0.10 2026-02-28T16:47Z Put a flag on the play - no assignment in Frugalese
