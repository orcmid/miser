<!-- m230900c.md 0.3.0              UTF-8                         2026-03-07
     ----1----|----2----|----3----|----4----|----5----|----6----|----7----|--*
...
<table border="0" width="100%">
  <tr>
    ...
    <td width="27%" height="6" valign="middle" align="right">
      ...
      <br />
      <small><small>
        0.3.0 2026-03-07T02:06Z<!-- MAINTAIN THIS MANUALLY -->
      </small></small>
      </td>
  </tr>
</table>
...
ob might be tied to an item in the [MOb](../../2011/04/m110400.html) domain
look-up table.

- [1. HashFlag Assistance to Comparison](#1-hashflag-assistance-to-comparison)
- [2. The Abstract Comparison of Obs](#2-the-abstract-comparison-of-obs)
- [3. The Algorithmic Comparison of Obs](#3-the-algorithmic-comparison-of-obs)
- [4. The Benefit of HashFlags for Expedited Comparison](#4-the-benefit-of-hashflags-for-expedited-comparison)
- [5. The Quest to Eliminate Colliding Duplicates](#5-the-quest-to-eliminate-colliding-duplicates)
  - [5.1 Going Under the Covers](#51-going-under-the-covers)
  - [5.2 Knowing Where From](#52-knowing-where-from)
  - [5.3 Considerations](#53-considerations)
  - [5.4 Can Greed Be Good?](#54-can-greed-be-good)
...
To eliminate a duplicate, it must be known where the references for *x* and
*y* were obtained so those references can be made to refer to the same
ob-cell, discarding a duplicate on a match  In (3) we have pointers *x* and
*y* to the ob-cells as parameter values, but there is no knowledge of where
those pointers were obtained.

Occassionaly it is known where pointere came from: going into recursive steps
`q(a(x), a(y))` and `q(b(x), b(y))` in (2) and its counterpart (3) above.

Replace those with `qa(x, y)` and `qb(x, y)` so it becomes known what
ob-cells are going to be checked.  Now there are to ob-cells that can be
adjusted when their corresponding ob-cell parts are found to represent the
same ob.
...
is somehow preferable to retain.

Reference count adjustments are not reflected. On exit, one
goes up, one goes down.  That can be handled in preferable(*x*, *y*).
...
* The recursive descent through the two obs is difficult for pairs.  It may
  be crucial in practice to find a tail-recursive construction for these
  cases.  There might be a binary tree traversal in support of that
  \[[Knuth1997: 2.3.1](https://orcmid.github.io/bib/compsci.htm#Knuth1997)\].
  It will take more careful investigation of the trade-off.
...
\[[Knuth1997](../../../../../bib/authors.htm#Knuth1997)\]
Knuth, Donald E.  Section 2.3.1, Traversing Binary Trees, in *The Art of
Computer Programming, vol.1: Fundamental Algorithms*, ed.3.  Addison-Wesley
(Reading, MA: 1968, 1973, 1997).  ISBN 0-201-89683-4
...
<!--
  ...
  0.3.0  2026-03-07T02:06Z Streamlining
  0.2.1  2026-02-20T04:22Z Smoothing and tightening
