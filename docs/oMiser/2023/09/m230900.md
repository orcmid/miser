<!-- m230900.md 0.1.8               UTF-8                         2026-01-25
     ----1----|----2----|----3----|----4----|----5----|----6----|----7----|--*
...
<table border="0" width="100%">
  <tr>
    ...
    <td width="27%" height="6" valign="middle" align="right">
      ...
      <br />
      <small><small>
        0.1.8 2026-01-25T20:04Z<!-- MAINTAIN THIS MANUALLY -->
      </small></small>
      </td>
  </tr>
</table>
...
alternative to garbage collection.  Becausse of the next question, there are
only 32-bits left for the HashFlag, so ***24-bits*** has to be enough.  That's
oddly reminiscent of the address space of the original IBM System 360 address
space and the mistaken use of the additional 8 bits by applications that would
eventually get burned.

- Is a 24-bit (3-byte) field adequate for an optional reference count? (A
collateral question as part of keeping x64 ob-cells to 4 64-bit words).  The
answer to this question is ***NO***.  A 32-bit word is required because the
AddRef and Release actions on the reference count must be atomic and there
are intrinsics for the 32-bit-aligned-word case.  There also needs to be some
humility around the assumption that x64 will never be exceeded in the lifetime
of this oMiser design and implementation.
...
- There is a related consideration with regard to short-lived ob-cells, ones
  ...
  hashes (and actions on reference counts?) might be deferred until a sweep.
    This is all speculative. I'd heard that Henry Baker may have done
  something similar in Chicken Scheme (https://www.call-cc.org/) and his
  clever C Language approach with continuations has been described at
  <https://dl.acm.org/doi/10.1145/214448.214454>.
    Henry and I talked about this notion back at the first AAAI conference
  when I was thinking of CP/M and Z-80 implementatins where hardware stacks
  were usable. I was already thinking of Miser but had not formulated it yet.
    The trick in rapid initial ob-cell constructions on the stack is avoiding
  dragging so many no-longer reachable intermediate ob-cells that the run-
  time stack overflows.
    With Just-In-Time (JIT) generation of machine code along with good tail-
  recursion handling, there might be a reasonable time before sweeps are
  required. There might still be pathological cases, such as when lists are
  reversed/mirror-imaged.  And there might also be tricks to rescue those.
  There must be some intensive testing of proposed solutions.
